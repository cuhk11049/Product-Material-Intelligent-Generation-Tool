...
interface StyleSelectorProps {
  ...
  onUploadCustom?: (file: File) => Promise<string | null>; 
}

export const StyleSelector: React.FC<StyleSelectorProps> = ({ selectedStyleId, onSelect, onUploadCustom }) => {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const uploadedUrl = await onUploadCustom(file);
  ...
  const handleCustomCardClick = () => {
    ...
    if (hasCustomImage) {
        const customStyle: StyleOption = {
            id: CUSTOM_STYLE_ID,
            name: '自定义',
            imageUrl: customPreviewUrl!, // 使用当前的预览图作为 URL
            description: '用户上传的参考图'
        };
        onSelect(customStyle);
    } else {
        ...
        >
            <div className="aspect-square relative flex items-center justify-center">
                {isUploading ? (
                    ...
                    hasCustomImage ? (
                        <>
                            <img src={customPreviewUrl!} className="w-full h-full object-cover" />
